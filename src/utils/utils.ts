...
export const isUndefined = (num: any) =>
    Object.prototype.toString.call(num) === '[object Undefined]';
...
export const randomStrings = () =>
    new Array(4)
        .fill('')
        .reduce((p) => p.concat(String.fromCharCode(97 + Math.ceil(Math.random() * 25))), '');
...
export const saveRefenerceData = async (data: any) => {
    const { data: formId, code } = await request(`/api/case/ws_case/params/`, {
        method: 'post',
        data,
    });
    if (code !== 200) return;
...
export const SERVER_INFO_CONFIG = [
    'ip/sn',
    'distro',
    'cpu_info',
    'memory_info',
...
export const useServerConfigArray = () => {
    ...
    return [
        ['IP', 'ip/sn'],
        [formatMessage({ id: 'report.server.distro', defaultMessage: '机型' }), 'distro'],
        ['CPU', 'cpu_info'],
        [formatMessage({ id: 'report.server.memory_info', defaultMessage: '内存' }), 'memory_info'],
        [formatMessage({ id: 'report.server.disk', defaultMessage: '磁盘' }), 'disk'],
        [formatMessage({ id: 'report.server.ether', defaultMessage: '网络' }), 'ether'],
        ['OS', 'os'],
        ['Kernel', 'kernel'],
        ['GCC', 'gcc'],
        ['Glibc', 'glibc'],
    ...
    if (Array.isArray(list)) {
        ...
        list.forEach((item) => {
            if (keyList.indexOf(item[onceName]) < 0) {
                !lodash.isUndefined(item[onceName]) && keyList.push(item[onceName]);
            }
        });
                ...
                const osName = arr.map((r: any) => {
                    ...
                    const children = param.concat(
                        list
                            .filter(
                                (l: any) =>
                                    l[onceName] === key && l[typeName] === item && l[os_name] === r,
                            )
                            .map((k) => ({
                                value: k.id,
                                label: listRender(k),
...
// 机器
export const GROUP_MANAGE = !BUILD_APP_ENV
    ? isOpenSource
        ? list_cn['fixed_pool.server']
        : list_cn['aligroup.server']
    : aligroupServer;
export const CLOUD_MANAGE = !BUILD_APP_ENV
    ? isOpenSource
        ? list_cn['elastic_pool.server']
        : list_cn['aliyun.server']
    : aliyunServer;
// 基线
export const aligroupServer_baseline = isOpenSource
    ? list_cn['fixed_pool.baseline']
    : list_cn['aligroup.baseline'];
export const aliyunServer_baseline = isOpenSource
    ? list_cn['elastic_pool.baseline']
    : list_cn['aliyun.baseline'];
// 单机 | 集群
export const aligroupServer_standalone = isOpenSource
    ? list_cn['fixed_pool.standalone']
    : list_cn['aligroup.standalone'];
export const aliyunServer_standalone = isOpenSource
    ? list_cn['elastic_pool.standalone']
    : list_cn['aliyun.standalone'];
export const aligroupServer_cluster = isOpenSource
    ? list_cn['fixed_pool.cluster']
    : list_cn['aligroup.cluster'];
export const aliyunServer_cluster = isOpenSource
    ? list_cn['elastic_pool.cluster']
    : list_cn['aliyun.cluster'];
...
// 机器
export const GROUP_MANAGE_en = !BUILD_APP_ENV
    ? isOpenSource
        ? list_en['fixed_pool.server']
        : list_en['aligroup.server']
    : aligroupServer_en;
export const CLOUD_MANAGE_en = !BUILD_APP_ENV
    ? isOpenSource
        ? list_en['elastic_pool.server']
        : list_en['aliyun.server']
    : aliyunServer_en;
// 基线
export const aligroupServer_baseline_en = isOpenSource
    ? list_en['fixed_pool.baseline']
    : list_en['aligroup.baseline'];
export const aliyunServer_baseline_en = isOpenSource
    ? list_en['elastic_pool.baseline']
    : list_en['aliyun.baseline'];
// 单机 | 集群
export const aligroupServer_standalone_en = isOpenSource
    ? list_en['fixed_pool.standalone']
    : list_en['aligroup.standalone'];
export const aliyunServer_standalone_en = isOpenSource
    ? list_en['elastic_pool.standalone']
    : list_en['aliyun.standalone'];
export const aligroupServer_cluster_en = isOpenSource
    ? list_en['fixed_pool.cluster']
    : list_en['aligroup.cluster'];
export const aliyunServer_cluster_en = isOpenSource
    ? list_en['elastic_pool.cluster']
    : list_en['aliyun.cluster'];
// ----------------- end 替换上面的逻辑 ------------------

...
const getVars = (vars: any) => {
    ...
    let arr = vars;
    if (Object.prototype.toString.call(vars) === '[object String]') arr = JSON.parse(vars);
    const list = arr?.filter(({ name, val }: any) => name && val);
    if (list.length > 0) return JSON.stringify(list);
    return undefined;
};

const ConfEditDrawer: React.ForwardRefRenderFunction<AnyType, AnyType> = (
    { onOk, selectedRowKeys }: any,
    ref: any,
) => {
        ...
        form.setFieldsValue({
            var: handle
                ? JSON.stringify(
                      content.map((i: any) =>
                          Object.fromEntries(
                              Object.entries(i).filter(([name, val]) => name !== 'error'),
                          ),
                      ),
                      null,
                      4,
    ...
    return (
        ...
        >
            ...
            >
                ...
                {!data.batch && (
                    <Form.Item name="alias" label={<FormattedMessage id="suite.alias" />}>
                ...
                <Form.Item name="repeat" label={<FormattedMessage id="TestSuite.repeat" />}>
                    <InputNumber
                        style={{ width: '100%' }}
                        min={1}
                        step={1}
                ...
                {!handle ? (
                    <Form.Item name="var" rules={[{ validator: validFunction }]}>
                        <Input.TextArea
                            autoSize={{ minRows: 4 }}
                            style={{ width: '100%' }}
                            placeholder="格式：key=value, description，多个换行"
                        />
                    </Form.Item>
                ) : (
                    <Form.List name="var">
                            ...
                            return (
                                <Space direction="vertical">
                                    {fields.map((field: any, index: any) => (
                                        <Form.Item key={field.key} name={[field.name, 'error']}>
                                            <Space align="center">
                                                <Form.Item
                            ...
                            <span>
                                ...
                                <Popover
                                    ...
                                    content={
                                        <div>
                                            <p>
                                                <FormattedMessage id="TestSuite.is_certified.ps" />
                                            </p>
                                        </div>
                                    }
                ...
                {!data.batch && (
                    <Form.Item name="description" label={<FormattedMessage id="TestSuite.desc" />}>
                        <Input.TextArea
                            rows={4}
                            placeholder={formatMessage({ id: 'TestSuite.desc.placeholder' })}
                        />
                    </Form.Item>
                )}
            </Form>

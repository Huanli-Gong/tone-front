    ...
    const columns: any = [
        {
            ...
            filterIcon: () => (
                <FilterFilled style={{ color: params.name ? '#1890ff' : undefined }} />
            ),
        ...
        },
        {
            ...
            sorter: true,
            render: (_: any, row: any) => (
                <ColumnEllipsisText ellipsis={{ tooltip: true }} children={row.gmt_created} />
            ),
        },
        {
            ...
            sorter: true,
            render: (_: any, row: any) => (
                <ColumnEllipsisText ellipsis={{ tooltip: true }} children={row.gmt_modified} />
            ),
        },
        {
            ...
            dataIndex: 'creator',
            filterIcon: () => (
                <FilterFilled style={{ color: params.creator ? '#1890ff' : undefined }} />
            ),
            filterDropdown: ({ confirm }: any) => (
                <SelectDrop
                    confirm={confirm}
                    onConfirm={(val: any) => {
                        setParams({ ...params, creator: val });
                    }}
                />
            ),
        },
        {
            ...
            filterIcon: () => (
                <FilterFilled style={{ color: params.update_user ? '#1890ff' : undefined }} />
            ),
            filterDropdown: ({ confirm }: any) => (
                <SelectDrop
                    confirm={confirm}
        ...
        },
        {
            ...
            render: (text: any, record: any) => {
                return (
                    <Space size="small">
                        <span
                            className={styles.fail_detail_operation}
                            onClick={() => hanldeEdit(record)}
                        >
                        ...
                        >
                            <span className={styles.fail_detail_operation}>
                        ...
                        switch (order) {
                            case undefined:
                                if (field === 'gmt_created')
                                    return setParams({ ...params, gmt_created: undefined });
                                if (field === 'gmt_modified')
                                    return setParams({ ...params, gmt_modified: undefined });
                            case 'descend':
                                if (field === 'gmt_created')
                                    return setParams({ ...params, gmt_created: '-gmt_created' });
                                if (field === 'gmt_modified')
                                    return setParams({ ...params, gmt_modified: '-gmt_modified' });
                                break;
                            case 'ascend':
                                if (field === 'gmt_created')
                                    return setParams({ ...params, gmt_created: '+gmt_created' });

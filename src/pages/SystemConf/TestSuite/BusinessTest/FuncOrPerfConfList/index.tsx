...
export default forwardRef(({ id, type: test_type, domainList }: any, ref: any) => {
    ...
    const queryDeleteAll = async () => {
        try {
            const data =
                (await queryConfirm({ flag: 'pass', case_id_list: selectedRowKeys.join() })) || {};
            if (data.code === 200) setDeleteVisible(true);
            else setDeleteDefault(true);
            setDeleteObj({});
        } catch (e) {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (text: any) => (
                <ColumnEllipsisText ellipsis={{ tooltip: true }} children={text || '-'} />
            ),
        },
    ...
    return (
        <div className={`${styles.warp} case_table_wrapper`}>
            {test_type === 'performance' && (
                ...
                >
                    <TabPane tab="Test Conf" key="1" />
                    <TabPane tab={<FormattedMessage id="TestSuite.conf.metric" />} key="2" />
                </Tabs>
            )}
            {innerKey == '1' ? (
                <CommonTable
            ...
            >
                <div style={{ color: 'red', marginBottom: 5 }}>
                    ...
                    {deleteObj.name
                        ? formatMessage(
                              { id: 'TestSuite.conf.name.delete.warning' },
                              { data: deleteObj.name },
                          )
            ...
            {selectedRowKeys.length > 0 && (
                <div className={styles.batch}>
                    ...
                    <Space>
                        <Button onClick={queryDeleteAll}>
                            <FormattedMessage id="operation.batch.delete" />
                        </Button>
                        <Button type="primary" onClick={editAll}>
                            <FormattedMessage id="operation.batch.edit" />
                        </Button>
                    </Space>
                </div>
            )}

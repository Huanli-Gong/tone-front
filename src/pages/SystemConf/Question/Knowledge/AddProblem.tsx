...
import { Drawer, Form, Space, Button, Input, Row, Col, Typography, message } from 'antd'
...
const AddReasonDrawer: React.ForwardRefRenderFunction<AnyType, AnyType> = (props, ref) => {
    ...
    const handleOk = async () => {
        try {
            ...
            if (fetching) return
            setFetching(true)
            const { code, msg } = !source ? await postProblem(values) : await putProblem({ problem_id: source?.id, ...values })
            setFetching(false)
    ...
    const handleAddKeyword = () => {
        keywordInput && form.setFields([{ name: 'keyword', value: (form.getFieldValue('keyword') || []).concat(keywordInput) }])
        setKeywordInput(undefined)
    }
                ...
                <Form.Item
                    ...
                    rules={[{
                        validator(rule, value, callback) {
                            if (!value) return Promise.reject(`推荐值不能为空`)
                            if (Object.prototype.toString.call(+ value) !== '[object Number]') {
                                return Promise.reject(`推荐值需为数字。`)
                            }
                            else {
                                if (+ value < 0) {
                                    return Promise.reject(`推荐值不能小于0`)
                                }
                            }
                        ...
                        <Col span={20}>
                            <Space direction='vertical' style={{ width: '100%' }} size={0}>
                                <Input
                                    ...
                                    onChange={(evt: React.ChangeEvent<HTMLInputElement>) => setKeywordInput(evt.target?.value)}
                                    allowClear
                                />

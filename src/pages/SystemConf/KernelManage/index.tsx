    ...
    const columns = [{
        ...
        render: (_: any) => (
            <>
                ...
                <Typography.Text>{_ ? <FormattedMessage id="kernel.release.version"/> : <FormattedMessage id="kernel.temporary.version"/>}</Typography.Text>
            </>
        ),
        ...
        dataIndex: 'enable',
        width: 100,
        render: (_: any) => <><Badge status={_ ? 'success' : 'error'} /><Typography.Text>{_ ? <FormattedMessage id="kernel.enable"/> : <FormattedMessage id="kernel.stop"/>}</Typography.Text></>,
        ...getRadioFilter(pageParams, setPageParams, 

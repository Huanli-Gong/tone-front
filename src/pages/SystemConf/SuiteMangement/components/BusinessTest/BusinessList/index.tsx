...
import { Popover, Tooltip, Space, message, Popconfirm, Modal, Button, Checkbox, Typography } from 'antd';
  ...
  // 批量选择操作
	const rowSelectionCallback = (info: any)=> {
		...
		if (business_id && selectedRowKeys.length) {
			if (selectedSuites.businessId && business_id !== selectedSuites.businessId) {
        // 重置上一个子表格的选中状态
	...
	const onCancelRowSelection = ()=> {
		...
		setRestId(selectedSuites.businessId)
		// 重置当前表格的选中状态
		suiteTable.current?.onCancelRowSelection()
		setSelectedSuites({ businessId: '', selectedSuiteKeys: [] })
	}
	// 批量删除
	const deleteAll = () => {
		suiteTable.current?.deleteAll()
	}
	...
	return (
		<div className={styles.business_root}>
			<CommonTable className={styles.businessList}
				columns={columns}
				list={list}
				loading={loading}
				page={pageNum}
				...
				expandable={{
					expandedRowRender: (record: any) => {
						return <SuiteList business_id={record.id} ref={suiteTable} rowSelectionCallback={rowSelectionCallback} restId={restId} />
					},

...
import { Affix, Typography, Checkbox, Row, Button, Space, Tabs, message, Popconfirm, Modal } from 'antd';
...
import ConfEditDrawer from './ConfEditDrawer'
 ...
 * @returns 
 */
 export default forwardRef(({ id, type }: any, ref : any) => {
    const { TabPane } = Tabs;
    const { domainList, setSelectedRowKeys, setSelectedRow, selectedRowKeys , confDrawerShow, confRefresh, setConfRefresh } = useContext(TestContext)
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (_: any, row: any) => (
                <ButtonEllipsis title={row.description} width={100} isCode={true}>
                    <EditOutlined
    ...
    const handleEditDesc = async (data: any) => {
        const { code, msg } = await editCase(data.id, { description: data.description })
        if (code === 200) {
            message.success('操作成功')
...
keyboard={ false }
                ...
                footer={
                    ...
                    >
                        ...
                        <Button onClick={onCaseSubmit} type="primary" htmlType="submit" >
                            {drawerTitle.indexOf('新增') > -1 ? '确定' : '更新'}
                        </Button>
                    </div>
                }
            >
                    ...
                    <Row gutter={16}>
                        ...
                        <Col span={24}>
                            <Form.Item
                                ...
                                label="运行次数"
                                rules={[{ required: true, message: '请输入' }]}
                                initialValue={ type === 'performance' ? 3 : 1 }
                            >
                        ...
                        </Col>
                        {
                            ...
                            <Col span={24}>
                                <Button
                                    ...
                                    size="small"
                                    onClick={() => {
                                        let content = formCase.getFieldValue('var')
                                        if (!handle) {
                                            try {
                                        ...
                                        formCase.setFieldsValue({ var: handle ? JSON.stringify(content, null, 4) : JSON.parse(content) })
                                        setHandle(!handle)
                                    }}
                                >
                                                ...
                                                <div>
                                                    ...
                                                    <Button
                                                        ...
                                                        onClick={() => {
                                                            ...
                                                            formCase.setFieldsValue({ var: content })
                                                        }}
                                                        block
                                                    >
                                                        + 添加变量

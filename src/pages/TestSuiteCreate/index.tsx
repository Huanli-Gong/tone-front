...
import { ReactComponent as IsExpand } from '@/assets/svg/is_expand.svg'
...
export default (props: any) => {
    ...
    const getTestSuiteList = async () => {
        ...
        if (leftWsHasSuiteArr.length === 0 && ws && flag) {
            setLeftTableLoading(true)
            const { data: list } = test_type === 'business' ? await queryWorkspaceBusinessSuite({ ws_id: ws, scope: 'brief' }) : await queryWorkspaceSuiteList({ ws_id: ws, test_type, scope: 'brief_case',page_size: 100 })
            wsHasSuiteArr = list
        }
    ...
    const handleDetail = () => {
        ...
        //     }
        // )
        window.open(`/ws/${ws}/refenerce/1/?test_type=${test_type}&name=${delType}&id=${case_id_list}`)
    }

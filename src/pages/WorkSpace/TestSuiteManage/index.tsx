...
const SuiteManagement: React.FC<any> = (props) => {
    ...
    const view_type_content = (
        <div>
            <div>
                <FormattedMessage id="suite.view_type.type1" />
            </div>
            <div>
                <FormattedMessage id="suite.view_type.type2" />
            </div>
            <div>
                <FormattedMessage id="suite.view_type.type3" />
            </div>
        </div>
    );
            ...
            render: (_: any) =>
                _ ? (
                    <Tooltip placement={'bottomLeft'} title={_}>
                        <Typography.Text style={{ width: 85 }} ellipsis>
                            {_ || '-'}
                        </Typography.Text>
                    </Tooltip>
                ) : (
        ...
        },
        {
            title:
                testType === 'functional' ? (
                    ...
                    <>
                        <FormattedMessage id="suite.view_type" />{' '}
                        <Tooltip title={view_type_content} placement="bottomLeft">
            ...
            render: (_: any, record: any) =>
                testType === 'functional' ? <></> : suiteChange(_, record),
        },
        {
    ...
    return (
        <TabCard
            title={
                <Tabs defaultActiveKey={testType} onChange={handleTab}>
                    <TabPane tab={<FormattedMessage id="functional.test" />} key="functional" />
                    <TabPane tab={<FormattedMessage id="performance.test" />} key="performance" />
                    {!BUILD_APP_ENV && (
                        <TabPane tab={<FormattedMessage id="business.test" />} key="business" />
                    )}
                </Tabs>
            }
            extra={
                <Access accessible={access.WsMemberOperateSelf()}>
                    <Button
                        ...
                        onClick={() => {
                            if (
                                ['functional', 'performance', 'business'].includes(
                                    fetchParams.test_type,
                                )
                            ) {
        ...
        >
            ...
            {fetchParams.test_type === 'business' && <BusinessTest />}

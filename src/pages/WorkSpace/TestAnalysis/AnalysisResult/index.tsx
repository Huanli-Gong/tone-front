...
const Report: React.FC = () => {
  ...
  const backTop = () =>
    ((document.querySelector('.ant-layout-has-sider .ant-layout') as any).scrollTop = 0);
  ...
  const handleData = async () => {
    ...
    resLen.map((item: any) =>
      queryCompareResultFn(item).then(({ code, data }) => {
        if (res.code === 500) {
          redirectErrorPage(500);
          return;
        }
  ...
  const handleCreatReportOk = () => {
    // suiteData：已选的
    saveReportDraw.current?.show({});
  };

  const createReportCallback = (reportData: any) => {
    // suiteData：已选的
    history.push({
      ...
      >
        ...
        <AnalysisWarpper style={{ width: group > 3 ? group * 390 : 1200 }}>
          <Col span={24}>
            ...
            <ResultContent>
              {environmentResult && JSON.stringify(environmentResult) !== '{}' && <TestEnv />}
              <ModuleWrapper style={{ position: 'relative' }} id="test_data" ref={testDataRef}>
                <SubTitle>
                  <span className="line" />
                  <FormattedMessage id="analysis.job.data" />
                </SubTitle>
        ...
        {ws_id && (
          <SaveReport ref={saveReportDraw} onOk={createReportCallback} allGroup={allGroupData} />
        )}
      </div>
    </ReportContext.Provider>
  );

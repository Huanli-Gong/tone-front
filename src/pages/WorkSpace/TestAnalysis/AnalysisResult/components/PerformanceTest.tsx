...
const GroupBarWrapper: React.FC<any> = (props) => {
  ...
  if (visible) {
    return (
      <GroupBar width={width} y={top - testOffset - floatRow.offsetTop}>
        <Summary style={{ border: 'none', paddingLeft: 18, paddingRight: 15 }}>
          <Group>
            <PerfGroupTitle gLen={groupLen}>
              <FormattedMessage id="analysis.comparison.group.name" />
            </PerfGroupTitle>
...
const ReportTestPref: React.FC<any> = (props) => {
      ...
      <TestItemFunc>
        <Space>
          <Button onClick={switchMode}>
            <FormattedMessage
              id={btnName === 'chart' ? 'analysis.chart.mode' : 'analysis.list.mode'}
            />
          </Button>
          {btn && (
            <Space>
              <Typography.Text>
                <FormattedMessage id="analysis.filter" />:{' '}
              </Typography.Text>
              <Select
                defaultValue="all"
                style={{ width: 200 }}
              ...
              >
                <Option value="all">
                  <FormattedMessage id="analysis.all" />
                </Option>
                <Option value="invalid">
                  <FormattedMessage id="analysis.invalid" />
                </Option>
                <Option value="volatility" title={formatMessage({ id: 'analysis.volatility' })}>
                  <FormattedMessage id="analysis.volatility" />
                </Option>
                <Option value="increase">
                  <FormattedMessage id="analysis.increase" />
                </Option>
                <Option value="decline">
                  <FormattedMessage id="analysis.decline" />
                </Option>
                <Option value="normal">
                  <FormattedMessage id="analysis.normal" />
                </Option>
              </Select>
            </Space>
          )}
  ...
  const handleArrow = (suite: any, i: any, conf: any) => {
    ...
    const endList = suite.conf_list.reduce((pre: any[], cur: any) => {
      if (cur.conf_id === conf.conf_id)
        return pre.concat({
          ...cur,
          metric_list: cur.metric_list
            .reduce((p: any[], c: any) => {
      ...
      setDataSource(
        dataSource.map((item: any) => {
          let conf_list = item.conf_list.map((conf: any) => {
            return {
              ...conf,
              metric_list: conf.metric_list.filter((metric: any) =>
                filterResult(metric.compare_data, value),
              ),
            };
          });
          return {
                      ...
                      item.conf_list.map(
                        (conf: any, cid: number) =>
                          !!conf.metric_list.length && (
                            <div key={cid}>
                              <TestConf>
                                <ConfTitle gLen={group}>
                                  <FormattedMessage id="analysis.TestConf/metric" />
                                </ConfTitle>
                                {allGroupData?.map((cont: any, i: number) => (
                                  <ConfData gLen={group} key={i}>
                                    {i !== baseIndex ? (
                                      <Row justify="space-between">
                                        ...
                                        <Col span={12}>
                                          <RightResult>
                                            ...
                                            <span
                                              onClick={() => handleArrow(item, i, conf)}
                                              style={{
                                                margin: '0 5px 0 3px',
                                            ...
                                            >
                                              {sortKeys.includes(conf.conf_id) &&
                                              sortRef.current === i ? (
                                                <IconArrowBlue />
                                              ) : (
                                                <IconArrow />
                                            ...
                                            <Tooltip
                                              ...
                                              title={
                                                <span style={{ color: 'rgba(0,0,0,0.65)' }}>
                                                  <FormattedMessage id="analysis.the.greater.difference" />
                                                  <br />
                                                  <FormattedMessage id="analysis.rules" />：<br />
                                                  <FormattedMessage id="analysis.decline" />
                                                  &gt;
                                                  <FormattedMessage id="analysis.increase" />
                                                  &gt;
                                                  <FormattedMessage id="analysis.fluctuation" />
                                                  &gt;
                                                  <FormattedMessage id="analysis.invalid" />
                                                </span>
                                              }
                                      ...
                                      <Typography.Text style={{ color: 'rgba(0,0,0,0.45)' }}>
                                        {allGroupData.length > 1 ? (
                                          <FormattedMessage id="analysis.benchmark" />
                                        ) : (
                                          <FormattedMessage id="analysis.result" />
                                        )}
                                      </Typography.Text>
                              ...
                              <div style={{ border: '1px solid rgba(0,0,0,0.10)' }}>
                                <PrefData>
                                  <PrefDataTitle gLen={group}>
                                    <EllipsisPulic title={conf.conf_name} />
                                  </PrefDataTitle>
                                  {(conf.conf_compare_data || conf.compare_conf_list || []).map(
                                    (objItem: any, idx: number) => {
                                      const selected_ws_id =
                                        group_jobs?.filter((jobObj: any) =>
                                          jobObj?.job_list?.includes(objItem.obj_id),
                                        )[0].ws_id || wsId;
                                      return;
                                ...
                                {conf.metric_list.map((metric: any, idx: number) => (
                                  <PrefMetric key={idx}>
                                      ...
                                      <Row justify="space-between">
                                        ...
                                        <Col span={8}>
                                          <Row justify="end">
                                            <RightResult>
                                              (
                                              {`${toPercentage(metric.cv_threshold)}/${toPercentage(
                                                metric.cmp_threshold,
                                              )}`}
                                              )
                                            </RightResult>
                                          </Row>
                                    ...
                                    {Array.isArray(metric.compare_data) &&
                                      !!metric.compare_data.length &&
                                      metric.compare_data
                                        ?.slice(0, group)
                                          ...
                                          <MetricText gLen={group} key={i}>
                                            <Row justify="space-between">
                                              <Col span={item && item.compare_result ? 12 : 20}>
                                                <Row justify="start">
                                                  <EllipsisPulic title={conversion(item)}>
                                                    <Typography.Text
                                                      style={{ color: 'rgba(0,0,0,0.65)' }}
                                                    >
                                                      {conversion(item)}
                                              ...
                                              {item && item.compare_result && (
                                                <Col span={12}>
                                                  <Row justify="end">
                                                    <RightResult>
                                                      <span
                                                        className={handleColor(item.compare_result)}
                                                      >
                                                        {item.compare_value || '-'}
                                                      </span>
                                                      <span

...
const SuiteManagement: React.FC<any> = () => {
    ...
    const submitSuite = async (data: any) => {
        ...
        outId
            ? flush()
            : listParams?.page_num == 1
            ? flush()
            : setListParams((p: any) => ({ ...p, page_num: 1 }));
    };
    ...
    const remOuter = async (params: any) => {
        ...
        setListParams((p: any) => ({
            ...p,
            page_num: Math.ceil((data.total - 1) / listParams.page_size) || 1,
        }));
        message.success(formatMessage({ id: 'operation.success' }));
    ...
    const columns: any = [
        {
            ...
            filterIcon: () => (
                <FilterFilled style={{ color: listParams?.name ? '#1890ff' : undefined }} />
            ),
            render: (_: any, row: any) => <ResizeTag {...row} />,
        },
        {
        ...
        },
        {
            ...
            width: 100,
            filterIcon: () => (
                <FilterFilled style={{ color: listParams?.creator ? '#1890ff' : undefined }} />
            ),
            filterDropdown: ({ confirm }: any) => (
                <SelectDrop
                    confirm={confirm}
        ...
        },
        {
            ...
            width: 100,
            filterIcon: () => (
                <FilterFilled style={{ color: listParams?.update_user ? '#1890ff' : undefined }} />
            ),
            filterDropdown: ({ confirm }: any) => (
                <SelectDrop
                    confirm={confirm}
        ...
        },
        {
            ...
            filterIcon: () => (
                <FilterFilled style={{ color: listParams?.description ? '#1890ff' : undefined }} />
            ),
            filterDropdown: ({ confirm }: any) => (
                <SearchInput
                    confirm={confirm}
                ...
                row.source_tag !== '系统标签' && (
                    <Space>
                        ...
                        <Popconfirm
                            title={
                                <div style={{ color: 'red' }}>
                                    <FormattedMessage id="job.tags.delete.prompt" />
                                </div>
                            }
                            placement="topRight"
    ...
    return (
        <SingleTabCard
            ...
            extra={
                <Button key="3" type="primary" onClick={newSuite}>
                    <FormattedMessage id="job.tags.create" />
                </Button>
            }
        >
            ...
            <Drawer
                ...
                title={
                    outId ? (
                        <FormattedMessage id="job.tags.edit.tag" />
                    ) : (
                        <FormattedMessage id="job.tags.create.tag" />
                    )
                }
                width={376}
                ...
                <Spin spinning={validateStatus == 'validating'}>
                    ...
                    >
                        ...
                        >
                            <Input.TextArea
                                rows={3}
                                placeholder={formatMessage({ id: 'job.tags.remarks.placeholder' })}
                            />
                        </Form.Item>
                    </Form>
                </Spin>

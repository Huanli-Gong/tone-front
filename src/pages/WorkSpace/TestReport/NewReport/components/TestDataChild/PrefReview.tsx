...
const getSortNum = (compare_result: string) =>
  new Map([
    ['decline', 0],
    ['increase', 1],
    ['normal', 2],
...
>>>>>>> anolis/feat/share_report_record_type
  ...
  const handleConditions = (value: any) => {
    setFilterName(value);
    let dataSource = isOldReport
      ? handleDataArr(lodash.cloneDeep(child), baseIndex)
      ...
      <TestItemFunc>
        <Space>
          {btn && (
            <Space>
              <Typography.Text>
                <FormattedMessage id="report.filter" />:{' '}
              </Typography.Text>
              <Select
                defaultValue="all"
              ...
              >
                ...
                <Option value="invalid">
                  <FormattedMessage id="report.invalid" />
                </Option>
                <Option value="volatility" title={formatMessage({ id: 'report.volatility' })}>
                  <FormattedMessage id="report.volatility" />
                </Option>
                <Option value="increase">
                  <FormattedMessage id="report.increase" />
                </Option>
                <Option value="decline">
                  <FormattedMessage id="report.decline" />
                </Option>
                <Option value="normal">
                  <FormattedMessage id="report.normal" />
                </Option>
              </Select>
            </Space>
          )}
  ...
  const renderShare = (conf: any, group_jobs: any) => {
    ...
    return arr.map((item: any, idx: number) => {
      ...
      const selected_ws_id =
        group_jobs?.filter((jobObj: any) => jobObj?.job_list?.includes(item.obj_id))[0].ws_id ||
        wsId;
      return (
  ...
  // suite遍历
  const RenderSuite = () => {
    return Array.isArray(perData.list) && !!perData.list.length ? (
      perData.list.map((suite: any, id: number) => (
        <TestSuite key={suite.suite_id}>
          <SuiteName>
            <Typography.Text
              style={{
                display: 'inline-block',
                textIndent: containerScroll?.left > 50 ? containerScroll?.left - 50 : 0,
              }}
            >
              {suite.suite_name}
            </Typography.Text>
              ...
              suite.conf_list && !!suite.conf_list.length ? (
                suite.conf_list.map((conf: any, cid: number) => (
                  <div key={conf?.conf_id}>
                    <TestConf>
                      <ConfTitle gLen={groupLen} style={{ marginLeft: btnState ? 39 : 0 }}>
                        <FormattedMessage id="report.conf/metric" />
                      </ConfTitle>
                          ...
                          {i !== baselineGroupIndex ? (
                            ...
                            >
                              ...
                              <RightResult>
                                ...
                                <span
                                  onClick={() => handleArrow(suite, conf, i)}
                                  style={{
                                    margin: '0 5px 0 3px',
                                ...
                                >
                                  {sortKeys.includes(conf.conf_id) && sortRef.current === i ? (
                                    <IconArrowBlue />
                                  ) : (
                                    <IconArrow />
                                  )}
                                ...
                                <Tooltip
                                  ...
                                  title={
                                    ...
                                    >
                                      <FormattedMessage id="report.performance.test.and.baseGroup" />
                                      <br />
                                      <FormattedMessage id="report.rules.as.follows" />
                                      ：<br />
                                      <FormattedMessage id="report.decline" />
                                      &gt;
                                      <FormattedMessage id="report.increase" />
                                      &gt;
                                      <FormattedMessage id="report.little.fluctuation" />
                                      &gt;
                                      <FormattedMessage id="report.invalid" />
                                    </span>
                                  }

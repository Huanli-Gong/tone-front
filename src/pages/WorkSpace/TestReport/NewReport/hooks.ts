...
export const CreatePageData = (props: any) => {
  ...
  const handleData = async (data: any) => {
    ...
    try {
      resLen.map((item: any, i: number) =>
        queryCompareResultFn(item).then((res) => {
          if (res.code === 200) {
            setLoading(false);
  ...
  React.useEffect(() => {
    if (saveReportData.template) getTemplate();
  }, []);
  /*
   *** 默认和自定义模版的切换
   */
      ...
      if (JSON.stringify(obj) !== '{}') {
        if (obj.perf_item && !!obj.perf_item.length) {
          if (!!perf_data_result.length) {
            for (let res = obj.perf_item, m = 0; m < res.length; m++) {
              // 自定义domain分组
              if (res[m].is_group) {
                // 是否有组
                let listParent: any = [];
                for (let domain = res[m].list, a = 0; a < domain.length; a++) {
                  //遍历组下面的项
                  let list: any = [];
                  let conf_list: any = [];
                  for (let suite = domain[a].list, b = 0; b < suite.length; b++) {
                    //遍历项下面的suite
                ...
                let list: any = [];
                let conf_list: any = [];
                for (let suite = res[m].list, b = 0; b < suite.length; b++) {
                  //遍历项下面的suite
                  perf_data_result.map((item: any, idx: number) => {
        ...
        if (obj.func_item && !!obj.func_item.length) {
          if (!!func_data_result.length) {
            for (let res = obj.func_item, m = 0; m < res.length; m++) {
              // 自定义domain分组
              if (res[m].is_group) {
                // 是否有组
                let listParent: any = [];
    ...
    const groupArr = compare_groups.reduce((pre: any, cur: any, idx: number) => {
      ...
      if (perf_data_result && !!perf_data_result.length) {
        const perfAll = perf_data_result.reduce((pa: any, c: any) => {
          ...
          if (all) return pa + all;
          return pa;
        }, 0);

        const perfCount = countCase(
          perf_data_result,
          'compare_count',
          { all: 0, decline: 0, increase: 0 },
          idx,
        );
        compare.perf_data = {
          perfAll,
  ...
  const queryReport = async () => {
    ...
    if (!templateSource || JSON.stringify(templateSource) === '{}') {
      if (Object.prototype.toString.call(tmpl_id) === '[object Number]') {
        ...
        if (code === 200) templateSource = data;
      }
    }
    let perf_item: any = [];
    let func_item: any = [];

...
const SearchPageList: React.FC<any> = (props: any) => {
    ...
    const getListData = async (query: any) => {
        try {
            ...
            const res =
                (await querySearchList({ search_key: keyword, search_type: tabKey, ...query })) ||
                {};
            const { data = {} }: any = res;
            if (
                res.code === 200 &&
    ...
    const handleClick = (item: any) => {
        if (item.id && item.test_suite_id) {
            ...
            a.href = `/ws/${ws_id}/suite_search/conf_Details?suite_id=${
                item.test_suite_id
            }&case_id=${item.id}&suite_name=${encodeURIComponent(
                item.suite_name,
            )}&conf_name=${encodeURIComponent(item.name)}`;
            a.click();
        }
    };
    ...
    const matchTextColor = (params: string) => {
        if (params && keyword) {
            ...
            const newStr: any = params.replace(
                reg,
                `<span style='color: #f00;opacity:1'>${keyword}</span>`,
            );
        ...
        <div className={styles.searchList}>
            {dataSource.map((item: any) => (
                <div className={styles.searchList_item} key={item.id}>
                    ...
                    <span className={styles.subTitle}>{matchTextColor(item.suite_name)}</span>
                    <div className={styles.content_tag}>
                        <span>{matchType(item.run_mode, formatMessage)}</span>
                        <Space>
                            {item.test_type && (
                                <div className={styles.type_tag}>
                                    {matchType(item.test_type, formatMessage)}
                                </div>
                            )}
                            {item.domain_name_list &&
                                item.domain_name_list.split(',').map((key: any, i: number) => (
                                    <div className={styles.type_tag} key={i}>
                                        {key}
                                    </div>
                                ))}
                        </Space>
                    ...
                    <div className={`${styles.content} ${styles.content_remarks}`}>
                        <Typography.Paragraph
                            ...
                            style={{ marginBottom: 0 }}
                        >
                            <FormattedMessage id="test.suite.remarks" />：
                            {matchTextColor(item.description)}
                        </Typography.Paragraph>
                    </div>
                </div>
            ))}
            ...
            {total !== 0 && pageSize >= total && (
                <div className={styles.little_data_tips}>
                    <FormattedMessage id="test.suite.it.over" />
                </div>
            )}
            {total === 0 && (
                <div className={styles.no_data_tips}>
                    ...
                    <p>
                        <FormattedMessage id="test.suite.failed.to.find" />
                    </p>
                    <div>
                        <FormattedMessage id="test.suite.please.try.to.search.content" />
                    </div>
                </div>
            )}

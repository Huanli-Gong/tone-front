...
export default memo((props: any) => {
    ...
    const handleServerTypeChange = useCallback(({ target }) => {
        setServerType(target.value);
    }, []);

    const handleServerObjectTypeChange = useCallback((value) => {
        setServerObjectType(value);
        setMask(false);
        form.setFieldsValue({
            server_object_id: undefined,
            ip: undefined,
    ...
    return (
        <>
            {!mask && (
                ...
                >
                    <Radio.Group value={serverType} onChange={handleServerTypeChange}>
                        <Radio value={'pool'}>
                            <FormattedMessage id="select.suite.the.server.pool" />
                        </Radio>
                        {server_type !== 'aliyun' && run_mode === 'standalone' && (
                            <Radio value={'custom'}>
                                <FormattedMessage id="select.suite.self.owned.server" />
                            </Radio>
                        )}
                    </Radio.Group>
                </Form.Item>
                        ...
                        <span className="ant-radio ant-radio-checked">
                            <span
                                className={`ant-radio-inner ${
                                    !multipInfo.serverPool ? styles.serverItemUncheckeds : ''
                                }`}
                    ...
                    {server_type !== 'aliyun' && run_mode === 'standalone' && (
                        <label className="ant-radio-wrapper" onClick={() => handleHideMask('self')}>
                            <span className="ant-radio ant-radio-checked">
                                <span
                                    className={`ant-radio-inner ${
                                        !multipInfo.selfServer ? styles.serverItemUnchecked : ''
                                    }`}
                                    style={{ borderColor: '#d9d9d9' }}
                                />
                            </span>
            ...
            <Row style={{ marginBottom: 8 }} className={'drawer_padding'} gutter={8}>
                {serverType === 'pool' ? (
                    <>
                        <Col span={10}>
                            <Select
                            ...
                            >
                                <Select.Option value={'ip'}>
                                    <FormattedMessage id="select.suite.random" />
                                </Select.Option>
                                {server_type === 'aliyun' && run_mode === 'standalone' ? (
                                    <>
                                        <Select.Option
                                            value={'instance'}
                                            title={formatMessage({ id: 'select.suite.instance' })}
                                        >
                                            <FormattedMessage id="select.suite.instance" />
                                        </Select.Option>
                                        <Select.Option
                                            value={'setting'}
                                            title={formatMessage({ id: 'select.suite.setting' })}
                                        >
                                            <FormattedMessage id="select.suite.setting" />
                                        </Select.Option>
                                    </>
                                ) : (
                                    <Select.Option value={'server_object_id'}>
                                        <FormattedMessage id="select.suite.server_object_id" />
                                    </Select.Option>
                                )}
                                <Select.Option value={'server_tag_id'}>
                                    <FormattedMessage id="select.suite.server_tag_id" />
                                </Select.Option>
                            </Select>
                        </Col>
                        <Col span={14}>
            ...
            <Form.Item
                ...
                label={
                    <QusetionIconTootip
                        ...
                        desc={`${
                            settingType === 'suite'
                                ? formatMessage({ id: 'select.suite.repeat.tootip1' })
                                : formatMessage({ id: 'select.suite.repeat.tootip2' })
                        }`}
                    />
                }
                className={'drawer_padding'}
            >
                <InputNumber
                    ...
                    max={10000}
                    placeholder={formatMessage({
                        id: multipInfo.repeat ? 'select.suite.multiple.values' : 'please.enter',
                    })}
                />
            </Form.Item>

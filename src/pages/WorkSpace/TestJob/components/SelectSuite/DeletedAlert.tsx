...
const getMessage = (keyStr: string, source: any[]) => {
    if (['ips', 'instance_name'].includes(keyStr)) {
        return source?.map(({ ip, sn }: any) => {
            if (ip || sn) return `${ip || sn}`;
            return `${ip}/${sn}`;
        });
    }
...
const DeletedAlert: React.FC<IProps> = (props) => {
    ...
    return list.map((item: any) => (
        <BasicAlert
            ...
            message={
                <Space wrap>
                    {getMessage(item, sources?.[item]).map((i: any) => (
                        <Typography.Text key={i}>{i}</Typography.Text>
                    ))}
                    <Typography.Text>
                        {intl.formatMessage({
                            id: `select.suite.${isDelete ? 'removed' : 'existent'}`,
                        })}
                    </Typography.Text>
                </Space>
            }

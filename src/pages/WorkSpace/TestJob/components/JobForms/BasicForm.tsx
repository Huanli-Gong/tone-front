...
export default (props: FormProps) => {
    ...
    React.useEffect(() => {
        ...
        if (JSON.stringify(template) !== '{}') {
            const { project, baseline, project_id, baseline_id, baseline_job, baseline_job_id } =
                template;
    ...
    useEffect(() => {
        if (projectList.length > 0) {
            if (!form.getFieldValue('project')) {
                ...
                if (projectObj) form.setFieldsValue({ project: projectObj.id });
            }
        ...
        >
            {'job_name' in contrl && (
                <Form.Item
                    name="name"
                    label={
                        contrl.job_name.alias || (
                            <FormattedMessage id={`job.form.${contrl.job_name.name}`} />
                        )
                    }
                    rules={[
                        {
            ...
            {'project' in contrl && (
                <Form.Item
                    name="project"
                    label={
                        contrl.project.alias || (
                            <FormattedMessage id={`job.form.${contrl.project.name}`} />
                        )
                    }
                >
                    <Select
                        ...
                        filterOption={(inputValue, option: any) =>
                            option.label.indexOf(inputValue) >= 0
                        }
            ...
            {(['functional', 'performance'].includes(test_type) ||
                ['functional', 'performance'].includes(business_type)) &&
                'baseline' in contrl && (
                    <Form.Item
                        name="baseline"
                        label={
                            contrl.baseline.alias || (
                                <FormattedMessage id={`job.form.${contrl.baseline.name}`} />
                            )
                        }
                    >
                        <Select
            ...
            {'baseline_job' in contrl && (
                <Form.Item
                    ...
                    label={
                        contrl.baseline_job.alias || (
                            <FormattedMessage id={`job.form.${contrl.baseline_job.name}`} />
                        )
                    }
                >
                    <Select

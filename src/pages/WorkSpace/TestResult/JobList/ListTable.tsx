...
const ListTable: React.FC<IProps> = (props) => {
  ...
  const handleDelete = async ({ id }: any) => {
    ...
    if ($page_num < pageQuery.page_num) setPageQuery({ ...pageQuery, page_num: $page_num || 1 });
    else listRefresh();
    countRefresh();
  };
  ...
  const columns: any = [
    ...
    },
    {
      ...
      render: (_: any, row: any) => (
        <span onClick={() => targetJump(`/ws/${ws_id}/test_result/${row.id}`)}>{row.id}</span>
      ),
    },
    {
        ...
        <OverflowList
          list={
            record?.tag_list?.map((item: any) => {
              return (
                <Tag color={item.tag_color} key={item.name}>
                  {item.name}
                </Tag>
              );
            }) || []
    ...
    },
    {
      title: (
        <QusetionIconTootip
          ...
          desc={
            ...
            >
              <li>
                <FormattedMessage id="ws.result.list.test_type.desc1" />
              </li>
              <li>
                <FormattedMessage id="ws.result.list.test_type.desc2" />
              </li>
            </ul>
          }
        />
      ...
      render: (_: any, record: any) => {
        ...
        if (lodash.isNull(result)) {
          return (
            <Row>
              <Col span={8} style={{ color: '#1890FF' }}>
                -
              </Col>
              <Col span={8} style={{ color: '#52C41A' }}>
                -
              </Col>
              <Col span={8} style={{ color: '#FF4D4F' }}>
                -
              </Col>
            </Row>
          );
    ...
    },
    {
      ...
      fixed: 'right',
      render: (row: any) => {
        const { state, created_from, creator } = row;

        const rerunDisabled =
          created_from === 'offline' || ['running', 'pending', 'pendding_q'].includes(state);
        return (
          <Space>
            <Access accessible={access.IsWsSetting()}>
              <Typography.Link
                disabled={rerunDisabled}
                onClick={() => !rerunDisabled && rerunRef.current?.show(row)}
              >
                <FormattedMessage id="operation.rerun" />
              </Typography.Link>
            </Access>
            ...
            <ViewReport dreType="left" ws_id={ws_id} jobInfo={row} origin={'jobList'} />
          </Space>
        );
      },
    },
  ...
  const handleMoreDeleOk = async () => {
    ...
    setPageQuery((p: any) => ({
      ...p,
      page_num: Math.ceil((dataSource?.total - selectedRowKeys.length) / p.page_size) || 1,
    }));
    countRefresh();
    // listRefresh()
  };
  ...
  const filterColumns = columns.filter((col: any) => {
    ...
    if (initialColumns[dataIndex]) {
      if (!initialColumns[dataIndex]?.disabled) return col;
    }
  });
  ...
  return (
    <Row style={basePadding}>
      <ResizeHooksTable
        ...
        refreshDeps={[access, ws_id, locale, sortOrder, initialColumns, dataSource]}
        name="test-job-list"
        loading={loading}
      ...
      {!loading && (
        <CommonPagination
          ...
          onPageChange={(page_num, page_size) =>
            setPageQuery((p: any) => ({ ...p, page_num, page_size }))
          }
        />
      )}

...
export default ({ jt_id, test_type, provider_name }: any) => {
    ...
    const { data, loading } = useRequest(() => querySettingConfig({ job_id, share_id }), {
        initialData: {
            test_config: [],
        },
        formatResult: (r) => {
            if (r.code === 200) return r.data[0];
            return {
                test_config: [],
            };
        },
    ...
    const { data: items } = useRequest(() => queryJobTypeItems({ jt_id, ws_id }), {
        initialData: [],
    });
        ...
        <div style={{ background: '#fff' }}>
            <Spin spinning={loading}>
                <div style={{ background: '#F5F5F5' }}>
                    ...
                    {/* <EnvPrepTable  { ...data } /> */}
                    <TestSuiteServerTable
                        data={data?.test_config}
                        server_schedule_rule={data?.server_schedule_rule}
                        testType={testType}

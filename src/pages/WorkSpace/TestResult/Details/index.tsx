...
import {
  ...
  deleteMyCollection,
  queryJobState,
} from '@/pages/WorkSpace/TestResult/services';
...
import { requestCodeMessage, matchTestType } from '@/utils/utils';
...
const TestResultDetails: React.FC<any> = (props) => {
  ...
  const queryJobDetails = async () => {
    ...
    if (dataSource?.state === 'running') setTab(2);
  ...
  const getJobState = async () => {
    ...
    const {
      data: { job_state },
      code,
    } = await queryJobState({ job_id, ws_id, share_id });
    if (code !== 200) return;
  ...
  const conversion = () => {
    if (!isNull(details?.baseline_job_id)) {
      return (
        <a href={`/ws/${ws_id}/test_result/${details?.baseline_job_id}`}>
          # {details?.baseline_job_id}
        </a>
      );
    }
  ...
  const handleCollection = async () => {
    const { msg, code } = !details.collection
      ? await addMyCollection({ job_id })
      : await deleteMyCollection({ job_id });
  ...
  // 判断是"离线上传"的数据
  const rerunDisabled =
    details?.created_from === 'offline' || CAN_STOP_JOB_STATES.includes(details.state);
  // 判断是"执行过程tab"
  ...
  const transProvider = (name: string) =>
    new Map([
      ['内网机器', 'aligroup'],
      ['云上机器', 'aliyun'],
      ['aligroup', 'aligroup'],
    ...
    <Spin spinning={loading} className={styles.spin_style}>
      {JSON.stringify(details) === '{}' ? (
        ...
        >
          ...
          >
            {!isSharePage && <BreadcrumbItem bottomHeight={4} {...details} />}
              ...
              <Row>
                <Col span={17}>
                  <Row
                    style={{
                      marginBottom:
                        details?.state !== 'pending' || isNull(details?.pending_state_desc)
                          ? 26
                          : 0,
                    }}
                  >
                    <Space>
                      ...
                      {details?.provider_name && (
                        <Tooltip
                          title={formatMessage({
                            id: 'ws.result.details.provider_name',
                          })}
                          placement="bottom"
                        >
                      ...
                      {details?.test_type && (
                        <Tooltip
                          title={formatMessage({
                            id: 'ws.result.details.test_type',
                          })}
                          placement="bottom"
                        >
                          <Tag color="#F2F4F6" style={{ color: '#515B6A', margin: 0 }}>
                            <FormattedMessage
                              id={`${matchTestType(details?.test_type)}.test`}
                              defaultMessage={details?.test_type}
                            />
                          </Tag>
                        </Tooltip>
                      )}
                        ...
                        message={
                          <span
                            dangerouslySetInnerHTML={{
                              __html: details?.pending_state_desc.replace(
                                /(\d+)/g,
                                `<a href="/ws/${ws_id}/test_result/$1" target="_blank">$1</a>`,
                              ),
                            }}
                          />
                        }
                  ...
                  <Row className={styles.test_summary_row}>
                    <RenderDesItem
                      name={formatMessage({
                        id: 'ws.result.details.creator_name',
                      })}
                      dataIndex={details?.creator_name}
                    />
                    <RenderDesItem
                      name={formatMessage({
                        id: 'ws.result.details.gmt_created',
                      })}
                      dataIndex={details?.gmt_created}
                    />
                    <RenderDesItem
                      name={formatMessage({
                        id: 'ws.result.details.finish_time',
                      })}
                  ...
                  <Row className={styles.test_summary_row}>
                    <RenderDesItem
                      name={formatMessage({
                        id: 'ws.result.details.project_name',
                      })}
                    ...
                    <RenderDesItem
                      name={
                        !isNull(details?.baseline_job_id)
                          ? formatMessage({
                              id: 'ws.result.details.baseline_job',
                            })
                          : formatMessage({
                              id: 'ws.result.details.baseline_test',
                            })
                      }
                    ...
                    <Col span={8}>
                      <Row>
                        ...
                        <Typography.Text
                          ...
                          style={{
                            width: `calc( 100% - ${widthStyle}px - 16px)`,
                          }}
                        >
                  ...
                  {details?.plan_instance_name && details?.plan_instance_id && (
                    <Row className={styles.test_summary_row}>
                      <RenderDesItem
                        name={formatMessage({
                          id: 'ws.result.details.plan_instance_name',
                        })}
                        dataIndex={details?.plan_instance_name}
                        isLink
                        onClick={() =>
                          history.push(
                            `/ws/${ws_id}/test_plan/view/detail/${details?.plan_instance_id}`,
                          )
                        }
                      />
                    </Row>
                  ...
                  <Row style={{ position: 'relative' }}>
                    ...
                    <div
                      style={{
                        ...TextStyle,
                        width: `calc(100% - ${widthStyle}px - 60px)`,
                      }}
                    >
                      {details?.note ? (
                        <Tooltip
              ...
              tabBarExtraContent={
                !isSharePage && (
                  <div style={{ display: 'flex', marginRight: 12 }}>
                    ...
                    <Access accessible={access.WsMemberOperateSelf(details?.creator)}>

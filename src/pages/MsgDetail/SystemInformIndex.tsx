...
import { Spin, Space, Typography, Badge, List, Avatar, Button, message, Row, Col, Pagination } from 'antd'
import { SysListItem } from './data.d';
import MsgEmpty from './Component/index'
import styles from './index.less';
import { handleRole } from '@/components/Role/index.js';
import { handleTypeWait,handleTypePassed,handleTypeRefused,jumpPage } from '@/components/RightContent/components/index.js'
            ...
            if (obj.action === 'set_ws_role') {
                ...
                return (
                    <div onClick={() => jumpRoute(item)}>
                        <Typography.Text ellipsis={true}>
                            <Typography.Text className={styles.list_user} >{obj.operator_info.user_name}</Typography.Text>
                            <Typography.Text className={styles.list_title}>把你添加为T-One</Typography.Text>
                            <Typography.Text className={styles.list_title} >{handleRole(obj.role_title)}</Typography.Text>
                        </Typography.Text>
                    </div>
        ...
        <Spin spinning={sysLoading}>
            {
                ...
                <div className={styles.read_father} style={{ height: layoutHeight - 183 }}>
                    <div className={styles.read_warp}>
                        <Space>
                            <Typography.Text className={styles.msg_text}>一共{msgNum?.apply_msg_total_num}条通知，其中{msgNum?.apply_msg_unread_num}条未读</Typography.Text>
                            <Typography.Text className={styles.msg_read}><Button type="link" onClick={handleAllSysRead} disabled={msgNum.apply_msg_unread_num === 0}>全部已读</Button></Typography.Text>
                        </Space>
                    </div>
                                ...
                                return (
                                    <List.Item>
                                        ...
                                        <Space>
                                            ...
                                            {!item.is_read 
                                                ? <Button style={{ marginLeft: 20 }} onClick={() => jumpRoute(item)}>查看</Button> 
                                                :  obj.action === 'set_ws_role' || obj.action === 'set_sys_role' || obj.action ==='remove' || (obj.status == 'refused' && obj.action == 'join' )
                                                ? <></>
                                                : <Button style={{ marginLeft: 20 }} onClick={() => jumpRoute(item)}>查看</Button>
                                            }
                                        </Space>

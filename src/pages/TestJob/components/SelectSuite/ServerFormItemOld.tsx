...
import { Form, Col, Radio, Row, Select, Input, Tag, InputNumber, Space,  } from 'antd'
...
export default (props: any) => {
    const {
        ws_id, server_type, form, onChange, show, batch,
    ...
    const standaloneServerRequest = async (page_num = 1) => {
        const { data, code } = await standloneServerList({ ws_id, state: ['Available', 'Occupied', 'Reserved'], page_num }) //, page_size : 2
    ...
    const clusterGroupRequest = async (page_num = 1) => {
        const { data, code } = await queryClusterGroupServer({ cluster_type: 'aliyun', ws_id, page_num })
    ...
    const dispatchRequest = async (page_num = 1) => {
        const { data, code } = await queryDispatchTags({ ws_id, run_mode, run_environment: server_type, page_num })
    ...
    const handleValidataCustomIp = async (custom_ip: any) => {
        ...
        if (custom_ip) {
            if (custom_channel) {
                const { code, msg } = await checkIpAndSn({ ip: custom_ip, channel_type: custom_channel })
                setLoading(false)
    ...
    return (
        <>
            ...
            {/* // <Row className={styles.drawer_padding} style={{ marginBottom: 8 }}>机器</Row> */}
            {
                (mask && batch) &&

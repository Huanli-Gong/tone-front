...
const OverflowList: React.FC<Iprops> = (props) => {
    ...
    const [currentCanShowCount, setCurrentCanShowCount] = React.useState<number | undefined>(
        undefined,
    );

    const reduceCoutWidth = React.useCallback(
        (entries: IntersectionObserverEntry[], index: number) =>
            entries
                .filter((_: IntersectionObserverEntry, idx: number) => idx <= index)
                .reduce(
                    (p: number, c: IntersectionObserverEntry) => (p += c.boundingClientRect.width),
                    0,
                ),
        [],
    );

    const oberverCallback = React.useCallback((entries: IntersectionObserverEntry[]) => {
        const count = entries.reduce(
            (pre: number, cur: IntersectionObserverEntry, index: number) => {
        ...
        <div ref={$dom} style={{ position: 'relative', userSelect: 'none', ...style }} {...rest}>
            ...
            {ellipsis && (
                ...
                >
                    ...
                    >
                        {listRender(
                            list.filter((_: any, index: number) => index < currentCanShowCount),
                        )}
                    </div>
                    ...
                    {currentCanShowCount - list.length !== 0 && (
                        <Tooltip
                            ...
                            title={listRender(
                                list.filter(
                                    (_: any, index: number) => index >= currentCanShowCount,
                                ),
                            )}
                        >
                            <span style={{ color: '#8c92a4', fontSize: 12, flexShrink: 0 }}>

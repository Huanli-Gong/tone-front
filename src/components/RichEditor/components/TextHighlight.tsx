...
import { ReactComponent as TextColor } from "../assets/text-highlight.svg"
...
const TextHighlight: React.FC<Record<string, any>> = ({ editor }) => {
    if (!editor) return <></>

    let active = ""

    for (const x of colors) {
        if (editor.isActive("highlight", { color: x })) {
            active = x
            break;
        }
    }
    ...
    return (
        <DorpdownMenu
            menu={
                <TextColorWrapper>
                    <DefaultColorItem
                        onClick={() => editor.chain().focus().toggleHighlight({ color: "rgb(38, 38, 38)" }).run()}
                    >
                        ...
                        <>
                            {
                                colors.map((i: string) => (
                                    <ColorItem
                                        key={i}
                                        bg={i}
                                        onClick={() => editor.chain().focus().toggleHighlight({ color: i }).run()}
                                    >
                                        {
                                            // editor.isActive('highlight', { color: i }) &&
                                            active === i &&
